# Attention-based Multi-modal YOLO 훈련 계획 (최종)

**목표**: 열상 + 야간투시 + Enhanced 가시광 3-way 입력으로 야간 Person 탐지  
**아키텍처**: Attention-based Late Fusion  
**하드웨어**: RTX 4090 24GB, Ryzen 7 7800X, 32GB DDR5
...
```
Phase 0: 환경 설정 및 데이터 준비
    ↓
Phase 1: 열상 단일 모달 Baseline 모델 훈련 (Plan C)
    ↓
Phase 2: Attention Fusion 3-way 모델 구현
    ↓
Phase 3: 사전학습 가중치 로드 및 Fine-tuning
    ↓
Phase 4: 하이퍼파라미터 최적화
    ↓
Phase 5: 최종 평가 및 배포
...
pip install timm einops  # Attention 모듈용
```
...
│   │   ├── nvg/          # 야간투시
│   │   ├── rgb/          # 원본 가시광
│   │   ├── rgb_enhanced/ # Enhanced 가시광
│   │   └── labels/       # YOLO 형식 라벨
│   └── yaml/
│       ├── thermal.yaml      # Phase 1용
│       └── multimodal.yaml   # Phase 3용
├── models/
│   ├── attention_fusion.py   # Attention 모듈
│   ├── multimodal_yolo.py    # 메인 모델
│   └── data_loader.py        # 데이터 로더
├── scripts/
│   ├── phase1_thermal.py     # Phase 1 훈련
│   ├── phase3_multimodal.py  # Phase 3 훈련
│   └── evaluate.py           # 평가
├── weights/                  # 학습된 가중치 저장
├── runs/                     # TensorBoard 로그
└── configs/
    └── hyperparams.yaml      # 하이퍼파라미터
```
...
path: ../data/daytime  # 데이터셋 루트
train: thermal/train   # 학습 이미지 폴더
val: thermal/val       # 검증 이미지 폴더
test: thermal/test     # 테스트 이미지 폴더
...
nvg_path: nvg
rgb_enhanced_path: rgb_enhanced
...
# Phase 1: 열상 단일 모달 Baseline 모델 훈련

**목적**: 열상만으로 Person 탐지 능력 확보 (Phase 3의 사전학습 가중치로 사용)
...
def train_thermal_baseline():
    """Phase 1: 열상 단일 모달 모델 훈련"""
    
    # YOLO11x 모델 로드 (COCO 사전학습 가중치)
    ...
    results = model.train(
        ...
        # Augmentation
        hsv_h=0.015,            # 색조 변화 (열상은 단색이므로 작게)
        hsv_s=0.4,              # 채도
        hsv_v=0.4,              # 명도
        degrees=10.0,           # 회전 각도
        translate=0.1,          # 이동
        scale=0.5,              # 스케일
        shear=0.0,              # 전단 변형
        perspective=0.0001,     # 원근 변형
        flipud=0.0,             # 상하 반전 (사람은 보통 안 함)
        fliplr=0.5,             # 좌우 반전
        mosaic=1.0,             # Mosaic augmentation
        mixup=0.1,              # Mixup augmentation
        
        # 정규화
        dropout=0.0,            # Dropout (YOLO는 보통 사용 안 함)
        
        # 성능 최적화
        amp=True,               # Automatic Mixed Precision (FP16)
        workers=8,              # 데이터 로더 워커 (CPU 코어 수)
        device=0,               # GPU 0번 사용
...
**주요 모니터링 지표**:
- `mAP50`: 50% IoU에서의 mAP (주요 지표)
- `mAP50-95`: IoU 50~95% 평균 mAP (엄격한 평가)
- `Precision`: 정밀도 (탐지한 것 중 실제 Person 비율)
- `Recall`: 재현율 (실제 Person 중 탐지한 비율)
- `Box Loss`: BBox 위치 정확도
- `Class Loss`: 클래스 분류 손실
...
class MultiModalYOLO(nn.Module):
    ...
    def __init__(self, 
        ...
        """
        Args:
            backbone_weights: YOLO 사전학습 가중치 경로
            fusion_dim: Fusion layer의 feature dimension
...
class MultiModalDataset(Dataset):
    """Multi-modal dataset for thermal + nvg + rgb_enhanced"""
    ...
    def __init__(self, 
        ...
        self.rgb_dir = self.data_root / split / 'rgb_enhanced'
...
# Phase 3: Multi-modal 모델 Fine-tuning 훈련
...
training:
  # Basic
  epochs: 100
  batch: 12              # RTX 4090 24GB 기준 (조정 가능)
  imgsz: 640             # 또는 1280 (고해상도)
  workers: 8
  device: 0
  seed: 42
  ...
  # Optimizer
  optimizer: 'AdamW'
  lr0: 0.0001            # Fine-tuning이므로 낮은 학습률
...
# - 약 4-6시간 (3개 백본이므로 Phase 1보다 오래 걸림)
```
...
# 2. Batch Size
batch: [8, 12, 16]  # 메모리 허용 범위 내

# 3. Image Size
imgsz: [640, 1280]  # 고해상도 실험
...
## Phase 2: 모델 구현
- [ ] attention_fusion.py 작성
- [ ] multimodal_yolo.py 작성
- [ ] data_loader.py 작성
- [ ] 모델 구조 테스트 (forward pass)
...
- mAP50: 0.85 ~ 0.92 (Baseline 대비 +10%)
- mAP50-95: 0.65 ~ 0.75 (Baseline 대비 +15%)
- Precision: 0.85+
- Recall: 0.80+
```
...
1. **열상**: 온도 기반 사람 탐지 (기본)
2. **야간투시**: 근적외선 디테일 추가
3. **Enhanced 가시광**: RGB 색상 정보 보완
4. **Attention Fusion**: 상황별 최적 모달리티 자동 선택
...
```python
# 체크 사항:
1. Learning rate 너무 높음 → 0.0001로 낮추기
2. 데이터 로더 확인 (라벨이 올바른지)
3. Phase 1 가중치가 제대로 로드되었는지 확인
...
```python
# 해결책:
1. Dropout 증가: 0.1 → 0.2
2. Weight decay 증가: 0.0005 → 0.001
3. 데이터 증강 강화
4. Early stopping patience 줄이기
```

## 4. 한 모달리티만 학습됨
```python
# Attention weights 모니터링
# LearnableModalityWeights 추가하여 각 모달리티 기여도 확인
# 필요시 modality-specific loss balancing
```

---

이제 이 문서대로 단계별로 진행하시면 됩니다! 🚀
추가 질문이나 막히는 부분 있으면 언제든 물어보세요!